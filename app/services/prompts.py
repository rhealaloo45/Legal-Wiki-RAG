...
- ASSUMPTIONS: State all assumptions explicitly. If you assume the client's role (e.g., "Acme is the customer"), say so.
...
- PROPER CITATIONS (CRITICAL): Cite inline with IEEE format [1], [2]. End with a "References" section: "[X] FileName.pdf, Page N, Clause/Section | Quote: <verbatim quote>". The "| Quote: ..." segment is OPTIONAL — include it only when a real verbatim excerpt is being quoted. When there is no verbatim quote for that reference, end the line after Clause/Section with no "| Quote:" segment at all. Never write a placeholder like "Quote: Not provided in excerpt" or "Quote: N/A" — a placeholder-quote is the same violation as a fabricated quote.
...
- PROPER CITATIONS (CRITICAL): Cite inline with IEEE format [1], [2]. End with a "References" section: "[X] FileName.pdf, Page N, Clause/Section | Quote: <verbatim quote>". Always include the page number if the context mentions it. The "| Quote: ..." segment is OPTIONAL — include it only when a real verbatim excerpt is being quoted. When there is no verbatim quote for that reference, end the line after Clause/Section with no "| Quote:" segment at all. Never write a placeholder like "Quote: Not provided in excerpt" or "Quote: N/A" — a placeholder-quote is the same violation as a fabricated quote.
