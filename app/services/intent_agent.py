...
# Brewline JVA1 and SteelLoop JVA3 questions). Suppress the between-only trigger
...
_RX_BETWEEN_PARTIES = re.compile(
    ...
    # parties - "the Judgment - Case No. 4/2000 between X and Y", "the
    # Plaint in the matter of Case No. 7/2000 between X and Y". Thirty
    ...
    # Periods are allowed through: a case reference carries its own dots ("Case No. 4/2000"), and
    r'[^?!;]{0,70}?\bbetween\b'
    ...
    # (confirmed live: "the NDA between Acme Passenger Ltd.
    # and Stratus Cell Intelligence" fell through to a comparison template
...
# A third false trigger: "board composition structured between Acme Power and
...
_RX_GOVERNANCE_APPROVAL = re.compile(
    ...
    # too — "without Acme's written approval" is a personnel-control clause a
...
# A case caption ("the Acme Sons vs Person 901 case", "X vs. Y") names ONE
...
# (confirmed live: "What did the defendants agree to transfer ... in the Acme
# Sons vs Person 901 case?" rendered a two-column table with "Not
...
def _question_precisely_names_a_document(question: str, session_id: str) -> bool:
    ...
    between Acme Passenger Ltd. and Stratus Cell
...
def check_clarification_node(state: QueryState) -> dict:
    ...
    # a fresh SHA-Gridline question was answered about "joint venture deadlock /
...
def generate_answer_node(state: QueryState) -> dict:
    ...
    # answer may have been drawn from the wrong one (confirmed live: a Gridline SHA
...
# Confirmed live on the 200-question set: "Case No. 7/2000" routed a
...
def _is_analytics_query(question: str) -> str:
    ...
    if (_RX_AGG_OP.search(q) and _RX_AGG_METRIC.search(q)
            ...
            # "the aggregate liability cap for Summit Gringotts Telecommunications
            # Private Limited and Person 008" returned a corpus statistic
...
# ventures involve Acme Power AND list sanctions exposure as an exit trigger,
...
# anchors on a capital letter, so a sentence-initial "From Summit Margie" keeps
# the "From".
...
# An instrument named as a possession of a party: "Massive's SLA", "the SLA
...
def _absent_instrument_answer(question: str, session_id: str,
    ...
    Asked to list the clauses in Summit Massive Motors' SLAs, the system answered
    ...
    Summit Massive Motors has no Service Level Agreement at all. It has a Master
...
def _question_parties(question: str) -> list[str]:
    ...
    Two shapes, because a lawyer writes both: "…agreements WITH Acme Steel"
    and "…agreements WHERE Acme Electronics is the lead strategic
    ...
    if wm:
        ...
        # "involve Acme Power and also list…" captured "Acme Power and", which
...
def _clause_precedent_answer(question: str, session_id: str) -> dict | None:
    ...
    if _how_many and hits:
        def _anchor_slice(hit) -> str:
            ...
            # when there is one ("Company076 Limited SHALL MAINTAIN true and
            # complete records ...").
        ...
        # most documents actually use. The Stark Retail/Oceanic
...
def _question_names_corpus_entity(question: str, session_id: str) -> bool:
    ...
    corpus holds the clause verbatim. Confirmed live, repeatedly: "What are Virtucon
    Tech's obligations regarding AI bias and discrimination?" took the standalone
    path on three separate runs — "Virtucon Tech" is not a gate keyword, and
    ...
    are needed ("Virtucon" lives only in filenames, not titles).
    """
    try:
        index = wiki._load_index(session_id)
        pages = index.get("pages", {})
...
def run_query_stream(question: str, session_id: str, target_doc: str = "",
    ...
    # load-bearing. "The total contract value of C02-X02-SOW-2026-001" trips
