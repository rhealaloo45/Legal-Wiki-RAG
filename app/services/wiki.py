...
- METADATA COMPLETENESS: Populate governing_law, jurisdiction, and parties whenever the \
  ...
  describe what is identifiable (e.g. "Acme Sons Private Limited; counterparty name redacted") \
...
Wrong: "SA1-Brightline – Term and Termination (Service Agreement)". \
Right: "Term and Termination – SA1-Brightline (Service Agreement)". \
  For court judgments: use first party's last name (e.g. "Person 900") \
...
Examples of DOCUMENT-SPECIFIC pages that MUST have the identifier in this order: \
  - Court judgments: Facts, Procedural History, Charges, Holding, Contentions, Relief, Costs \
    → "Facts – Person 900 (Court Judgment)" \
  - Contracts: Term, Termination, Payment, Fees, Indemnity, Liability, Scope of Services, \
    ...
    Parties, Assignment → "Term and Termination – SA1-Brightline (Service Agreement)" \
...
- PAGE TITLES: You MUST append the inferred Document Type in parentheses to EVERY page title. \
  ...
  a document identifier attached (e.g. "Facts – Person 900", "Term – SA1-Brightline") and some \
  ...
  document identifier — never the reverse. Wrong: "SA1-Brightline – Term". \
  Right: "Term – SA1-Brightline". \
  ...
  Examples: "Facts – Person 900 ({doc_type})", "Term – SA1-Brightline ({doc_type})", \
...
# terms (e.g. "field‑of‑use", "NDA‑Acme") that are NOT the plain ASCII
...
# ("Obligations of Receiving Party – NDA‑Acme – NDA") didn't match the
# real title ("...NDA-Acme...") only because of this character difference.
...
def _numbered_docs_in(question: str, doc_names) -> set[str]:
    ...
    A question can name MORE THAN ONE document this way (e.g. "compare Acme
    Brand Judgment 6 and Acme Brand Judgment 8") — iterate every match rather
    ...
    for num_match in _DOC_NAME_PATTERN.finditer(question):
        ...
        for doc_num in nums:
            ...
            # curated "document-165.pdf" must not also force-include
...
# _Redacted", "Court Case Document 5", "Acme Brand Judgment 6", etc.). The user's
...
# "document-165.pdf" AND the unrelated "Court_Case_Document_003
...
# somewhere ("MAT-0000-0000", "C02-X02-SOW-2026-001", "Case No. 18/2000"). Anchored
...
# page titles ("Definitions - SA1-Massive-Realty (Framework Supply
...
# an identifier, which is what keeps "Acme" (33 documents) out.
...
def _resolve_docs_by_page_identifier(question: str, session_id: str) -> set[str]:
    ...
    # matches three Service Agreements, "SA1-Massive-Realty" names one document.
...
def _detect_mentioned_files(question: str, pages: dict) -> set[str]:
    ...
    # 0b. Document CODE mention ("MAT-0000-0000", "C02-X02-SOW-2026-001").
...
# the exact inversion of what an answer needs. Measured on the Virtucon-Lumina MSA
...
# 48,33,33,333 figure never reached the model and the answer reported the cap
...
def get_context(question: str, session_id: str, target_doc: str = "", retrieval_hints: dict = None,
    ...
    if forced_pages:
        ...
        if not file_pages:
            ...
            # the vector search. Measured on Q21 ("the Service Agreement of Acme
...
# upper-case ("TSPL/LEGALOPS/2025/058", "MAT-0000-0000", "2025-CV-0041"), and it
...
def _verify_identifier_claims(answer: str, context: str) -> list[str]:
    ...
    quoting (e.g. "Case No. 11/2000" in a court-case answer) than a matter
...
# "N% / Rs. 40,441,445" — a milestone line carrying both its share of the
...
def _append_milestone_total(answer: str, context: str, question: str) -> str:
    ...
    Milestone schedules give each stage as "11% / Rs. 40,441,445" and never
    ...
    # "11% / Rs. 40,441,445" — while summing everything double-counts the
...
def _list_item_label(text: str) -> str:
    ...
    identity: "Insurance (Seller to maintain ... Rs. 75,588,327)" and "Insurance
...
def _drop_unverifiable_reference_quotes(answer: str, absent: list[str]) -> tuple[str, int]:
    ...
    Measured on Q60/Q64, whose ANSWERS were both exactly right (the correct Acme
    ...
    for m in _QUOTE_SPAN_RE.finditer(answer):
        ...
        for block_title in candidate_titles:
            ...
            if actual:
                ...
                # (e.g. "NDA-Acme" — a party-name identifier, not "NDA37"). If the
...
def _autocorrect_citation_attribution(answer: str, context: str) -> tuple[str, int]:
    ...
    title has no numbered identifier at all (e.g. "NDA-Acme", a party-name
...
def generate_answer(question: str, wiki_content: str, selected_titles: list, session_id: str, bm25_count: int = 0, page_selection_usage: dict = None, conversation_context: str = "", intent: str = "factual", unconfirmed_doc_reference: bool = False, scope_note: str = "", scope_warning: str = "", clause_directive: str = "", ambiguity_directive: str = "") -> dict:
    ...
    # correct answer is often genuinely short ("18 November 2025.", "Acme Steel
    ...
    # section rendered "Acme Brand Judgment 2" though the answer never once
    ...
    # citation like "document-189" would substring-match the generic
    ...
    # Injunction Framework – LO-Acme – Legal Opinion" carries "LO-Acme" — and the
    # model frequently cites bracket-style using THAT identifier ("[LO-Acme –
    ...
    # already uses for entity-matching, so a "LO-Acme"-style citation resolves to
    ...
    for t in referenced:
        if t not in seen:
            ...
            # ("LO-Acme") is short and distinctive by construction (>=4 chars,
...
_ENTITY_EXCLUDE = {
    ...
    # Found live-testing Voltmeter/Steelring SHA questions: "SHA-Omnimart –
    ...
    # distinctive identifier, out-competing the real single-token "voltmeter"
    ...
    # the real NDA (1 hit: "forgeline") and the real Arbitration Notice (1 hit)
    ...
    "under",
    # Found live-testing a DriveConnect/Voltmeter cross-document question:
...
def _identifier_boundary_re(ident: str) -> re.Pattern:
    ...
    "acme" is a substring of "sa-acme", so a citation reading
    "[From: SA-Acme - Service Agreement, Clause 8.3]" attributed the answer to
    the "Acme"-identified document as well — confirmed live, this is why an
    ...
    "-" is a non-word character, so \\btata\\b still fires inside "sa-acme".
...
def _doc_identifier_part(title: str) -> str:
    ...
    # some ingested titles have DocID-then-Topic order ("SHA-Omnimart –
    # Transfer Restrictions") instead of the expected Topic-then-DocID order
    # ("Transfer Restrictions – SHA-Omnimart"), so taking the last segment
    # grabs the generic topic phrase as if it were the identifier. Confirmed
    # live: this happened with BOTH "Transfer Restrictions" (Voltmeter case)
    # and "Information Rights" (Steelring case) — two different generic
    ...
    # prefix (e.g. "Company048 Portfolio LLP") also fails the ID-shape check,
...
def _question_mentions_known_entity(question: str, pages: dict) -> bool:
    ...
    "C01-X01") that no user ever types. Filenames keep the name the user
    ...
    right there — confirmed live for "Virtucon" (matches zero title-derived
    entities) and "Brackenfold" (same), each forced into the LLM disambiguation
...
def _corpus_doc_name_tokens(pages: dict) -> set[str]:
    ...
    filename keeps the name the user actually types ("Virtucon-Lumina"), so a
    question naming that party matches nothing in the title-derived entity set.
    Confirmed live: "Virtucon" appears in no page title in this corpus — the entity
    set holds "hyd-lex" — so every entity-based resolver returns empty for a
    question about Virtucon Tech, even though retrieval finds the documents easily.
    """
    tokens: set[str] = set()
    for page in pages.values():
        if not isinstance(page, dict):
...
def _question_names_distinctive_entity(question: str, pages: dict) -> bool:
    ...
    prompt three times in one thread, because "Virtucon" satisfies neither check
...
# for a party ("Brackenfold", "Virtucon"), the way people actually refer to a
# counterparty in conversation rather than by its full registered name.
# Confirmed live: "the required timeframe for Brackenfold to notify the
...
# holds exactly three documents mentioning "Brackenfold" by name — so
...
# ("Summit Proseware Digital") — this corpus's dominant bare-name pattern, one
...
# Summit Proseware Digital, Jan 2021", which fell all the way through to
...
# "and" — from "... between Acme Projects Limited and Mifflin Motors Ltd
...
# 113/2024", "Case No. 7/2000", "Case No. 8/2000". The filename writes the
...
def _narrow_by_question_tokens(question: str, candidate_docs: set[str],
    ...
    Confirmed live: "MAT-0000-0000 (IMG-4137 PPA)" resolves the matter number
    ...
    it. Same mechanism narrows "Guarantee agreement, Summit Proseware Digital"
    ...
    for m in _NARROW_TOKEN_RE.finditer(question):
        ...
        # "the Data Processing Agreement between Acme Capital Limited and
        # Massive Motors Limited" lost "data"/"processing"/"agreement" to that
    ...
    # Case No. 5/2000 ... dated 19 June 2024" carried both the case number
...
# A matter/reference number recited in the question ("MAT-0000-0000"). Looks
...
# "MAT-0000-0000" with different parties on each). Deliberately max_docs=1:
...
# notice period in MAT-0000-0000 (IMG-4137 PPA)" resolved to NOTHING under the
...
# MAT-0000-0000 sibling with different parties and a different notice period.
...
def _narrow_by_title_hint(session_id: str, candidate_docs: set[str], question: str,
    ...
    cluster for "Lucerne Mobility" but the question's other document
    ...
    ("Overview – Case No. 7/2000 (Court Judgment)") because ingest writes
...
def _resolve_docs_by_party(question: str, session_id: str, max_docs: int = 4) -> set[str]:
    ...
    ("Ollivander Entity"). So resolve it by a full-text
    ...
    like "Acme Steel Limited" hits many documents; the specific counterparty
    ...
    ("Acme Steel & Forgeline Metallurgy" → the NDA + arbitration notice +
    ...
    agreement, Summit Proseware Digital" — the party sits on 6 unrelated real
    ...
    if not config.USE_DATABASE:
        return set()
    # Suffix-derived names ("Wonka Metals LLC") are real, distinct legal
    ...
    if not candidates:
        # Phrase candidates first: a bare multi-word name ("Summit Proseware
        ...
        # single bare word ("Brackenfold") is never crowded out.
    ...
    # counterparty is Wideworld Metals - confidently, at 88%, about the wrong
    ...
    if best_n <= max_docs:
        ...
        # Agreement between Company005 Wingtip Limited and Hooli
    ...
    # second document being named, not noise. Confirmed live: "Acme Power"/
    # "Wonka Metals" (the SSA) won as smallest and returned alone, while
    # "Lucerne Mobility"/"Summit Margie Company078 Entity" (the judgment) were silently
    ...
    # over the bare-word fallback too: on "Guarantee agreement, Summit Proseware
    ...
    # Proseware Digital" amendment documents and got merged in as a false
    ...
    for name, docs in resolved:
        ...
        # live: "Section 5 of the Share Subscription Agreement between Acme
        # Elvara Limited and Umbrella Vehicles LLC" correctly pinned the Umbrella
        # SSA, then pulled in an unrelated Acme Elvara/Acme Elvara SSA on the
        ...
        # Detailed Judgment and Final Order - Case No. 5/2000 between
        # Company039 Rim Ltd and Umbrella InfoSystems LLC" pinned the
...
# A question can name SEVERAL documents by informal nickname — "the Hooli
# NDA, the Summit Cobalt NDA, and the Summit Initech EV NDA" — with no corporate
...
# only the Summit Cobalt NDA existed, though the Hooli and Summit Initech EV
...
# instrument, since a lawyer names a document that way ("the Hooli NDA")
...
# "the Hooli NDA" in caps, while the NAME must stay capitalised or this
...
# "From Summit Margie Company078 Entity Company" captures the "From" too.
...
def _resolve_docs_by_party_pair_index(question: str, session_id: str,
    ...
    legal names while a question says "Litware Capital".
...
def _resolve_docs_by_display_name(question: str, session_id: str) -> set[str]:
    ...
    generated Consulting Agreement document-094".
    ...
    titled 'Consulting Agreement document-094'" about
    ...
    for (sd,) in rows:
        ...
        # ("Consulting Agreement / document-094"),
...
def _resolve_docs_by_single_named_instrument(question: str, session_id: str,
    ...
    answer "compare the Hooli NDA and the Summit Cobalt NDA". A question
    naming ONE document that way had no resolver at all, so "the total contract
    value of the Sterling Global purchase agreement" fell through to an
...
def _resolve_party_within_family(question: str, session_id: str,
    ...
    handful of documents ("Acme Steel Limited", "Acme Sons Private Limited"),
    ...
    Measured on Q21 ("the termination clause of Service Agreement of Acme Steel
    Limited"): "Acme Steel Limited" appears in 7 documents — over the standalone
...
# rather than its second party ("operators of 'Acme Restart' and related
...
def _narrow_by_quoted_subject(question: str, session_id: str,
    ...
    that share the same plaintiff ("Acme Sons Private Limited" is the
    plaintiff in every Acme Brand Judgment) by naming the infringing subject
    in quotes instead of a second party — "the suit ... against operators of
    'Acme Restart' and related websites" has no second capitalised party for
    ...
    stripped ("AcmeRestart"), so the title ILIKE search below collapses the
    quoted phrase the same way — searching for "Acme Restart" WITH the space
    would match nothing.

    Confirmed on Q100: "Acme Sons Private Limited" alone spans every document
    ...
    the case number sat in Judgment 7). The quoted "'Acme Restart'" pins the
...
# live: an NDA's own page titles all read "NDA-Summit Margie", never
# "Litware", because ingest's short-naming picked one party) is still found,
...
# of Person 008 to Summit Gringotts...". Only the first (usually unsuffixed)
...
# person named only this way ("Person 008", no corporate suffix) left a
...
def _content_pair_supplement(session_id: str, tokens: list[str], full_names: list[str],
    ...
    both between "Acme Projects" and "Mifflin Motors". The SPA's every page
    title reads "... – Acme Projects/Mifflin (Share Purchase Agreement)" — a
    party-derived short name, so title search finds it. The Term Sheet's
    pages read "... – Matter Code04 (Term Sheet)" — an arbitrary code name
    ...
    isn't titled by party name. Full-text content confirms "Mifflin Motors"
    ...
    were each reduced from ("Acme Projects", not just "Acme") — the final
    content-verification step needs that fuller phrase, not the bare word;
    "Acme" alone appears in most of this corpus and confirms nothing.
    """
    ...
    # deal's Escrow, SPA, and Term Sheet filenames all say "Acme Projects"),
    ...
    # live: "acme"/"projects" alone pointed at the Escrow sibling (whose
    # filename literally reads "Acme Projects - Escrow - Agreement"), which
    ...
    # "Litware" alone left "Capital" (from "Litware Capital") in the narrowing
...
# "and" that joins the two parties of a single pair ("... Limited and Trey
# Travel Limited").
...
def _resolve_one_party_pair(question: str, session_id: str,
    ...
    # reduced from ("Acme Projects" for token "Acme"). _content_pair_supplement
...
def _resolve_docs_by_combinatorial_pairing(question: str, session_id: str,
    ...
    # live: "the notification obligations of Person 008 to X ... with
...
# named party wants ONE of its instruments (summarise "the Forgeline NDA") or a
...
def _carryover_scope(question: str, session_id: str) -> list[str]:
    ...
    # disambiguation prompt for the SHA-Gridline question landed on whatever
    ...
    if (_CARRYOVER_TYPE_RE.search(question.replace('_', ' '))
            ...
            # Confirmed live: "What are the biggest risks for Gringotts in this
...
def _carryover_subject_pivot(question: str, carried: list) -> bool:
    ...
    Confirmed live: asked "Does our Weyland litigation strategy satisfy the
    ...
    with nothing to do with Weyland — declining the statute correctly, then
...
def _carryover_comparative_set(question: str, session_id: str) -> list[str]:
    ...
    turn of a Acme confidentiality thread came back comparing waiver clauses in
...
def _enforce_question_family(scoped: dict, family: str | None,
    ...
    corpus: "the NDA between Acme Steel and Forgeline" resolved to the
    Acme-Forgeline *arbitration notice*, "the VitalSpring … *joint venture*" to an
    NDA, and "the *Services Agreement* between Acme Sons and its service provider"
...
def _resolve_docs_by_doc_type(question: str, session_id: str) -> set[str]:
    ...
    way — questions like "the Rejoinder in the Petition - Case No. 6/2000"
    whose document is filed under the corpus's own unexplained abbreviation
    ("MAT-0000-0000 RITPAN FINAL v2.pdf"), which no party or filename signal
...
# ("Acme Sons" is an umbrella name over the cap), _resolve_docs_by_party_pair sees
...
def _resolve_scope_uncorrected(question: str, session_id: str, pages: dict | None = None,
    ...
    # about "document-061.pdf" it matched on
    ...
    # a two-SSA comparison resolved "party" to the Acme Steel agreement alone
    ...
    # bare nickname ("the Hooli NDA, the Summit Cobalt NDA, and the Apex
    # Initech EV NDA") — no corporate suffix on any of them for _PARTY_NAME_RE
    ...
    if party_docs:
        ...
        # live: "Can Emberline use Thorvik's data to train AI models?" names
        # no instrument type, so this used to fall through — past the party
        # match that correctly found C02-X02-MSA/SOW/DPA — all the way to
        # carryover, which then answered from "Acme Brand Judgment 3", left
    ...
    if pair_docs:
        ...
        if len(pair_docs) > 1 and not _pair_segments:
            ...
            if date_docs and len(date_docs) == 1:
                ...
                # named, silently replacing a correct 5-document Acme Elvara
    ...
    # agreement", "the Sterling Global purchase agreement"). Placed here, after
    ...
    if _question_names_a_document(question, []) and _question_mentions_known_entity(question, pages):
        ...
        # the Forgeline NDA") pins the entity's dominant document — dropping the
    ...
    # "Acme Sons Private Limited" appearing across many NDAs/SHAs/JVAs/
    ...
    # are the biggest risks for Gringotts in this agreement?" names a party, so
    ...
    #     instrument ("the Service Agreement of Acme Steel Limited"). Falling
    ...
    if _fam_name and unresolved_party:
        ...
        #     ("operators of 'Acme Restart'") is the second signal those
...
def classify_query(question: str, session_id: str) -> dict:
    ...
    # corporate name ("Pinecrest Grid Private Limited"), or by bare
    # shorthand with no suffix at all ("Brackenfold"), whose name lives only in
    ...
    # share a common name. Confirmed live: "Brackenfold" resolves to exactly its
    ...
    # Neither is usable alone — "Acme Sons Private Limited" appears in 10
