...
# add a 30-day notice period silently turned Acme's UNILATERAL right into a
...
- LEGAL REVIEW NOTES (CRITICAL): After the drafted formulation(s), add a "**Legal Review Notes**" section as a Markdown table with columns: Source basis | Changed from source (if redrafting) | Fallback / negotiation point | Confirmation needed. Be SPECIFIC — "changed from source" must name the actual delta (e.g. "source Clause 8.3 gives Acme this right alone; this draft keeps it Acme-only" or, if something WAS changed, exactly what and why), not a generic "review alignment" note. If nothing changed from source, say so explicitly rather than leaving the cell blank.
- TABLE SYNTAX (CRITICAL): The table has EXACTLY two structural rows before any data — the header row and ONE delimiter row of dashes — then every row after that is real content. Never emit a second all-dashes row; a row of "---" cells is a delimiter, not a placeholder for missing data, and repeating it renders as a blank row above your real answer. Exact shape, four columns, no more and no fewer:
  ...
  | --- | --- | --- | --- |
  | Clause 8.3, Term and Termination – SA-Acme | Preserved Acme's unilateral right; added the 30-day notice period only | Consider a mutual notice cure window if requested in negotiation | Confirm no separate wind-down payment is owed beyond Services already performed |
