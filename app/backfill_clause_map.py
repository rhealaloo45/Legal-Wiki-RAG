...
"Return, Destruction, and Record Retention – Acme-Forgeline (NDA)" — which
...
def _q(s: str) -> str:
    """Single-quoted SQL literal. Doubling the quote is the standard escape and
    the only one needed here — every value is plain text (titles, headings,
    filenames), never an identifier."""
...
def _emit_sql(session_id: str, wiki_id: str, inserts: list[dict]) -> list[str]:
    ...
    against the LOCAL corpus and executed against Azure. If the two ever drift,
    a bare INSERT would happily create clause_map rows pointing at page titles
    that don't exist there — a map that looks populated and resolves to nothing.
    The WHERE EXISTS makes a drifted row simply not insert, and the counts
    printed either side show exactly how many landed.
    """
    out = [
        f"-- clause_map rows for session {session_id}",
        f"-- {len(inserts)} candidate rows, computed from the source PDFs locally.",
        "-- Safe to re-run: the DELETE below replaces this session's rows.",
        "",
        "BEGIN;",
        "",
        f"DELETE FROM clause_map WHERE session_id = {_q(session_id)};",
        "",
    ]
    for r in inserts:
        out.append(
    ...
    out += [
        "",
        "-- Verification: 'inserted' should be close to the candidate count above.",
        "-- A 0 here means the page titles on this DB differ from the local ones,",
        "-- i.e. the corpora have drifted and nothing was mapped.",
        f"SELECT count(*) AS inserted FROM clause_map WHERE session_id = {_q(session_id)};",
        "",
        "COMMIT;",
        "",
    ]
...
def main() -> None:
    ...
    ap.add_argument("--sql", metavar="FILE",
                    help="Write INSERT statements to FILE instead of touching the DB. "
                         "For environments where the Postgres port is unreachable "
                         "locally (corporate networks): compute the map here, where "
                         "the source PDFs live, then run the file with psql from "
                         "somewhere that can reach the database.")
    args = ap.parse_args()
    ...
    sql_out: list[str] | None = [] if args.sql else None
    if sql_out is not None:
        sql_out += ["-- Generated by backfill_clause_map.py --sql",
                    "-- Run with: psql \"$DATABASE_URL\" -f <this file>",
                    "SET client_encoding TO 'UTF8';", ""]

    for sid in sids:
        backfill_session(sid, args.dry, sql_out)

    if sql_out is not None:
        with open(args.sql, "w", encoding="utf-8") as f:
            f.write("\n".join(sql_out))
        print(f"\nWrote {args.sql}  ({os.path.getsize(args.sql)/1024:.1f} KB)")
        print("Run it from a host that can reach the DB:")
        print(f'  psql "$DATABASE_URL" -f {os.path.basename(args.sql)}')
