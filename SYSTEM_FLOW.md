...
- **Case-specific pages** (facts, holding, charges, procedural history, parties, contentions, relief) → prefixed with short case identifier: `"Facts – Person 900 (Court Judgment)"`
...
1. classify_intent
   ...
   - Entity+doc-type regex: "Voltco JV Agreement" → skips disambiguation.
...
2. disambiguation (wiki.classify_query)
   - Skipped if target_doc, is_followup, _question_names_a_document() matches,
     ...
     page titles (e.g. "Voltco", "Meridian", "Person 900").
...
4. retrieve (wiki.get_context)
   ...
   - Entity-aware retrieval: when no source_doc filename matches but
     ...
     those pages are force-included (fixes the "Voltco not covered" gap).
