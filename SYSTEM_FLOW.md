...
2. **Inputs**:
   ...
   - `relative_paths`: An optional JSON array representing folder paths for nested folder uploads (e.g., `["NDA/Acme_NDA.pdf"]`).
3. **Storage**: Files are saved to `data/uploads/` with the filename format: `{session_id}_{safe_relative_path}` (e.g., `24b74788_NDA_Acme_NDA.pdf`).
4. **Queueing**:
...
- **LLM Output**:
  ...
  References
  [1] Acme_NDA.pdf, Clause 4.2 | Quote: Payment shall be completed in net thirty (30) days...
...
- **LLM Output**:
  ```json
  {
    "value": "IP created under this agreement belongs strictly to Acme.",
    "confidence": 0.95,
    "quote": "All intellectual property rights... shall vest in Acme."
  }
  ```
...
- **NLP Inference**:
  ...
  - `detect_stance`: Uses keyword matching on the prompt (e.g., "acme-friendly", "vendor-favorable") to select a stance template (`acme_favorable`, `counterparty_favorable`, or `neutral`).
...
- **What is Sent to the LLM**: 
  ...
  - The stance instructions (e.g., "Draft from Acme's perspective...").
