...
4. **retrieve** (`wiki.get_context()`) — hybrid page selection, tuned by per-intent `retrieval_hints`. Entity-aware retrieval: when the question mentions a distinctive entity/party name from page titles (e.g. "Voltco"), those pages are force-included even if the source filename doesn't match. Numbered doc-type patterns ("service agreement 3") also match via enhanced `_detect_mentioned_files()`.
