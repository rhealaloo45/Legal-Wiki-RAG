...
Charges – Person 900 (Court Judgment)
```
...
5. **retrieve** (`wiki.get_context()`) — hybrid page selection (§6.2), tuned by per-intent `retrieval_hints` and constrained by the resolved scope (family filter narrows the vector search; `single_doc` scope force-pins retrieval to the resolved document(s)). Entity-aware retrieval: when the question mentions a distinctive entity/party name from page titles (e.g. "Voltco"), those pages are force-included even if the source filename doesn't match. Numbered doc-type patterns ("service agreement 3") also match via enhanced `_detect_mentioned_files()`.
